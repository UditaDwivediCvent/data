...
trigger updateMoodFrmCase on Case (after insert,after update) {
    ...
    if(testCase.size()>0){
        for(User u : [Select Manager_Email__c,Name from user where id in: testCase.keyset()]){
    ...
    boolean flag=false;
    if(caseMap.size()>0){
        for(Contact c : [Select Mood_Level__c,Profile_Name__c from Contact where id in : caseMap.keySet()]){
            c.Profile_Name__c = profileName;
            if(Trigger.isUpdate){
                ...
                if(beforeUpdateMood.Mood__c != caseMap.get(c.id).Mood__c){
                    ...
                    if((caseMap.get(c.id).Mood__c=='1 - Irate')||(caseMap.get(c.id).Mood__c=='2 - Frustrated')){
                        ...
                        c.Manager_Email__c = emailCase.get(c.id).Manager_Email__c;
