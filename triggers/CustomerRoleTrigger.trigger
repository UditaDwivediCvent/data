trigger CustomerRoleTrigger on Contact_Role__c (before insert, before update, after update) {
    ...
    if(!System.isBatch()){   
        ...
        **/
        if(customerConIds.size()>0){            
            ...
            //Get Community Useer
            for( User u : [Select Id,ContactId From User Where ContactId IN: customerConIds and isPortalEnabled=true]){
            ...
            for(Contact c : [Select Id, (Select Id From Cases where  LastModifiedDate =LAST_N_DAYS:180  AND (NOT (LastModifiedBy.Name like '%Sops%' OR LastModifiedBy.Name ='Sales Support India'))  Limit 1),(Select Id From Tasks Where ActivityDate=LAST_N_DAYS:180AND (NOT (LastModifiedBy.Name like '%Sops%' OR LastModifiedBy.Name ='Sales Support India')) Limit 1),
